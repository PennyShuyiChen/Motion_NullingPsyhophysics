function myout = WindyMoIndPosShiftNulling_hinge_SF(subj, stimType, vGabs, fname)
% WindyMoIndPosShiftNulling_hinge_SF.m
% Last modified: PSC - 03/17/2021
%       03/17 PSC data collected 

%% EXP - RUN! 
% Penny's Notes:
% 03142021 - Finalized for ecc & sf exp, with VIEWPixx display 

% 1) Ecc & SF dependencies exp task conditions: 'zeb', 'cfx', 'cfc'
    % zeb = zebra 
    % cfx = cuttlefish envelope (x-mouse on for envelope)
    % cfc = cuttlefish carrier (x-mouse off) - original iod condition 
% 2) 3SFs * 4Eccs * 20Repeats = 240 trials in total (3min break per 60 trials)
    % Min(Experiment time for one task condition) = 12sec * 240 + 3*3min = 57min 

% 3) Copy the lines to the console + press enter to run the experiment 

%       WindyMoIndPosShiftNulling_hinge_SF('JW', 'cfc', 1, 'cfcvp.mat');
%       WindyMoIndPosShiftNulling_hinge_SF('JW', 'zeb', 1, 'zebvp.mat');
%       WindyMoIndPosShiftNulling_hinge_SF('JW', 'cfx', 1, 'cfxvp.mat');

%% Larry's Notes :)
% Windy Motion Induced Position Shift Nulling
% valid stimulus types:  'zeb', 'cfo', 'cfx', 'cfh', 'itd', 'cfc(iod)', 'apt', 'pld'
% 'zebra', 'cuttlefishOrtho', 'cuttlefish Ortho with x-mouse on', 'cuttlefish', 'indieTwoD', 'indieOneD', 'aperature', 'plaid'

% $$$ conditions are zeb, cfx, and cfc/iod.  pld is the plaid control condition with both H and V gratings superimposed
...
% indie2D and indie1D (itd, iod/cfc) - Like the cuttlefish, except the carrier brownian walk is
...
% 14-Mar-2021   psc modified the code for eccentricity & SF dependecies
              % experiments (Display: VIEWPixx/3d)

% NOTES
% You must have PsychToolbox installed (psychtoolbox.org), VisTools in the
% path
%
...
switch ept.stimType
    case 'zeb'
        ept.TwoDimJitter = 1; % should be 1
    ...
    case 'cfx'
        ept.TwoDimJitter = 0;
        ept.carrierMotion = 1;  % 1 1 = carrier Motion for MIPS, 0 = non-drifting Gabors control
        ept.correctCarrierMotionForEnvlope = 0;
        ept.subjectChangesPhase = 0;
        ept.supjectChangesEnvelop = 1;
    case 'cfh'
...
%% Rig params - PSC
vdist = 38;% viewing distance in cm 
H_res = 1920; V_res = 1080; % monitor resolution
H_im = 20.5 .* 2.54; % effective image size in cm # info online
VA = rad2deg(atan((H_im/2) / vdist)); % Visual angle for 1/2 screen (H)
VA_pix = VA/(H_res/2); % how much visual degree a pixel extends dpp
1./VA_pix % show the ppd 
...
% Hinge-linear function + psychophysics scaling from Simoncelli&Freeman
% paper
for i = 1:E_ncond
    if E(i) <= 5
        RF_V(i) = 1;
        ...
        RF_V(i) = 0.26 * E(i); % psychphysics scaling 
        RF_MT(i) = 1* E(i);
    end
end
...
ngabors = [16,16,16,16,20,20,20,20,8,8,8,8];    % 16 for experiment
refRectSz = repmat([7],1,4); % reference rectangle inside circle of Gabors (fixation center)

%% Gabor initial parameters:
% Phase of underlying sine grating in degrees:
phase = 0;
% assme RF = 6 * sc, pre-sc of base SF 
scm = RF_V./6;
% fixed bandwidth in octave
BW = 1.4; % median of V1 simple cells 
% Frequency of sine grating in cpp:
baseSF = bwsig2freq(BW,scm);
% SF sets in cpd, then cpp
pSF = [bwsig2freq(BW,scm), 2*bwsig2freq(BW,scm),0.5*bwsig2freq(BW,scm)];
pSF = (pSF) * VA_pix;
...
SC = 1.3 .* sc; % 
% Contrast of grating:
ept.contrast = 0.1; % 10% Michelson contrast  
...
%% Experiment trial sequence and conditions - emat
        ...
        % 3)Spatial Constant  4&5) Tw & Th 7) ring radius (eccentricity)
        % 8) refRectSz
...
nSF = 3;%3
ntrialsblock = nSF * E_ncond * nrepeat;
nTrials = nSF * nrepeat * E_ncond;
block_divider = 60; % break time for every 60 trials 

makeMovie = 0;
showCursorAndCentroid = 0;  % only for movies and demos

% premat = (repmat(transpose([SC;r_R;TW;TH;E;refRectSz]),3,1));
 premat = (repmat(transpose([r_R;E;refRectSz]),3,1));
emat = ([pSF',SC',TW',TH',premat,ngabors']);
...
ept.stepSize =2; %2;      % ave ept.stepSize in pixels - determines target speed - set to 0 for no envelope motion
...
gamma = 1/2.1536; %original used 2.2 for SONY display; 2.1536 for VIEWPixx 
...
for j = 1:nTrials
    ...
    tw = emat(seq,4);
    th = emat(seq,5);
    ept.stimRad(j) = emat(seq,6);
    ept.ecc(j) = emat(seq,7);
    refRect = [centerX-emat(seq,8), centerY-emat(seq,8), centerX+emat(seq,8), centerY+emat(seq,8)];
    ept.ngabors = emat(seq,9); %%01202021 PSC

    gabOffsets = ept.stimRad(j).*unitcirc(ept.ngabors);
    backgroundOffset = [0,0,0,0];
    disableNorm = 1;
    preContrastMultiplier = 0.5;
    gabortex = CreateProceduralGabor(theWindow, tw, th,0,backgroundOffset,disableNorm,preContrastMultiplier); % theWindow, tw, th, 1
    carDrift = cumsum(dPhases.*(1./360)./(ept.freq(j)));
    % Initialize matrix with spec for all 'ept.ngabors' patches to start off
    % identically:
    ...
    for i = 1:totFrames
        ...
        % print the condition - for pilot 
%         Screen(theWindow,'FillRect',[bkCol, bkCol, bkCol,0]);
%         Screen('FrameRect', theWindow, [200, 200, 200], refRect, 1);
%         pilot = char(cellstr(strcat('Trial condition ', {' SF'}, num2str(ept.freq(j)), {' SC'}, num2str(sc),...
%             {' Ecc'}, num2str(ept.stimRad(j)))));
%         Screen(theWindow,'DrawText',pilot,50,50,255);
%         Screen('Flip', theWindow);
        ...
        if ept.correctCarrierMotionForEnvlope
            ...
            if ept.vGabs
             ...
             %phaseCorrect = phaseCorrection 
             %dt.phaseCorrection(:,i,j) = phaseCorrect;
        end
        ...
        elseif ept.subjectChangesPhase
            if ept.vGabs
                sPhaseChange = -deltaMouse(1) .* ept.freq(j) .* 360;
                yTargNew = yTargNew + deltaMouse(2);
            else
                sPhaseChange = deltaMouse(2) .* ept.freq(j) .* 360;
                xTargNew = xTargNew + deltaMouse(1);
            end
        ...
        elseif ept.supjectChangesEnvelop % need to move Gabors and then undo absolute phase change due to motion
            ...
            if ept.vGabs
                sEnvPhaseCorrection = deltaMouse(1).* ept.freq(j) .* 360; %  + deltaMouse(1))
            else
                sEnvPhaseCorrection = deltaMouse(2) .* ept.freq(j) .* 360; %  + deltaMouse(2)
            end
        else
            % mouse has no effect, so do nothing
        end
        %dt.sEnvPhaseCorrection(:,i,j) = sEnvPhaseCorrection;       
       %dt.sPhaseChange(:,i,j) = sPhaseChange;
    ...
    % add the block divider    
    if mod(j,block_divider) == 0 && j ~= length(emat)
        Screen('Close',gabortex);
        JustBkMouse(theWindow,180);       
        ShowCursor;
    end 
...
ept.freq = (ept.freq ./ VA_pix);%repmat(ept.freq, nTrials, 1);
...
if exist(fname, 'file')
    ...
    ep.TwoDimJitter = [ep.TwoDimJitter;ept.TwoDimJitter];
    ep.carrierMotion = [ep.carrierMotion;ept.carrierMotion];
    ep.correctCarrierMotionForEnvlope = [ep.correctCarrierMotionForEnvlope;ept.correctCarrierMotionForEnvlope];
    ep.subjectChangesPhase = [ep.subjectChangesPhase;ept.subjectChangesPhase];
    ep.supjectChangesEnvelop = [ep.supjectChangesEnvelop;ept.supjectChangesEnvelop];
