function myout = WindyMoIndPosShiftNullingDemo(subj, stimType, vGabs, fname)
% WindyMoIndPosShiftNulling.m
% PSC - 11.25.2020 - 3 SFs at each ecentricity
% Experiment conditions: 'zeb', 'cfx', 'iod'

%% EXP - RUN! copy the line to the console + Enter 
% WindyMoIndPosShiftNullingDemo('TT', 'cfx', 1, 'TT.mat');

% WindyMoIndPosShiftNulling_hinge_SF('LKC', 'cfx', 1, 'cfxexp.mat');

% WindyMoIndPosShiftNulling_hinge_SF('PSC', 'zeb', 1, 'zebexp.mat');
% WindyMoIndPosShiftNulling_hinge_SF('PSC', 'cfx', 1, 'cfxexp.mat');
%%
...
switch ept.stimType
    ...
    case 'cfx'
        ...
        ept.correctCarrierMotionForEnvlope = 0;
        ept.subjectChangesPhase = 1; % 0 
        ept.supjectChangesEnvelop = 0; % 1
    case 'cfh'
        ept.TwoDimJitter = 1;
        ept.carrierMotion = 1;  % 1 = carrier Motion for MIPS, 0 = non-drifting Gabors control
        ept.correctCarrierMotionForEnvlope = 0;
...
%% Rig params - PSC
vdist = 38;% original = 15.0 cm % 57.3/56.7
...
%ngabors = [16,16,16,16,16,16,16,16,16,16,16,16];
...
%pSF = [2*bwsig2freq(BW,scm), 2*bwsig2freq(BW,scm),2*bwsig2freq(BW,scm)];
pSF = (pSF) * VA_pix;
% scaling s = k/sf
k = scm .* baseSF;
k = repmat(k,1,3);
...
sc = k ./ pSF;
SC = 1.3 .* sc; % 
%SC =[4.3899    4.3899   11.4137   17.1205    4.3899    4.3899   11.4137   17.1205    8.7797    8.7797   22.8273   34.2410];


% Contrast of grating:
ept.contrast = 0.1; % 10% contrast  
