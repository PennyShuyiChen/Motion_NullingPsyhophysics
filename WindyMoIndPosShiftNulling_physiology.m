function myout = WindyMoIndPosShiftNulling_physiology(subj, stimType, vGabs, fname)
% WindyMoIndPosShiftNulling.m
% PSC - 11.25.2020 - Vertical test 
% TO RUN TESTS: e.g. WindyMoIndPosShiftNulling_physiology('testphys', 'zeb', 1, 'testphys.mat')
% TO RUN TESTS: e.g. WindyMoIndPosShiftNulling_hinge('spcfxPSC', 'cfx', 1, 'spcfxPSC.mat')
%WindyMoIndPosShiftNulling_hinge('LKC', 'zeb', 1, 'LKC.mat')
...
% Gabor numbers  
ept.ngabors = 1;    % 16 for experiment
...
pSF = (pSF) * VA_pix;
% Contrast of grating:
ept.contrast = 0.2; % 20% contrast% original = 10; 
...
%% Params for motion steps 

ept.stepSize = 5;      % ave ept.stepSize in pixels - determines target speed - set to 0 for no envelope motion
...
if ept.smooveSD
    ...
    SmooveKer=(1/(ept.smooveSD*sqrt(2*pi))).*exp(-x.^2./(2*ept.smooveSD^2)); % 1d-Gaussian kernel 
else
...
bkCol = 0.5;    % background color (grayscaled) 
...
gamma = 1/2.2; %gamma encoding/compression 
...
if ept.smooveSD
    for j = 1:nTrials %'same'returns the central part of the convolution that is the same size as A.
        temp(1,:,j) = conv(temp(1,:,j), SmooveKer, 'same'); % conv(input,kernel) 
        temp(2,:,j) = conv(temp(2,:,j), SmooveKer, 'same');
        temp(:,1,j) = [0; 0];
    end
end

% Target positions setup - no subject response (wind only) 
targCoordsNoResp = zeros(2,totFrames,nTrials);
targCoordsNoResp(:,:,:) = ept.stepSize*cumsum(temp, 2);    % what target positions would be if wind only (no response from Subj)
...
for j = 1:nTrials
    ShowCursor('Arrow');
    % read experiment params from emat 
    seq = eseq(j);
    ept.trial(j) = j;
    ...
    % Gabor pars + flags for CreateProcedualGabor 
    %gabOffsets = ept.stimRad(j).*unitcirc(ept.ngabors);gabOffsets
    gabOffsets =0.* ept.stimRad(j).*unitcirc(ept.ngabors);
    nonsymmetric = 0; % circular hull with fixed aspect-ratio
    backgroundOffset = [0,0,0,0]; % set to 0s if the background is set to grey - 0.5 
    disableNorm = 1; % the normalization will scale the Gab params based on SC, so turned off normf
    preContrastMultiplier = 0.5; % use this with properly gamma-corrected gray scale 
    ...
    % Wait for a click - initial screen
    ShowCursor('Arrow');
    ...
    tic % timer starts 
    for i = 1:totFrames % each trial 720 frames 
        ...
        deltaMouse = [x-xMouseOld; y-yMouseOld]; % the change in position
        ...
        % ***    2) Now do the subject mouse motion
        ...
        if ept.correctCarrierMotionForEnvlope
            ...
            if ept.vGabs % vGabs =  1 = vertical stripes 
        ...
        if ept.supjectChangesEnvelop && ept.subjectChangesPhase %zeb condition 
        ...
        % and, finally, update dstRects and phases - turn off the wind PSC
        %dstRects = CenterRectOnPointd(inrect, xTargNew+gabOffsets(:,1)', yTargNew+gabOffsets(:,2)');
        dstRects = CenterRectOnPointd(texrect, xTargNew+gabOffsets(:,1)', yTargNew+gabOffsets(:,2)');
