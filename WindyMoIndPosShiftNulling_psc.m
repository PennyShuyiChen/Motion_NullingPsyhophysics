function myout = WindyMoIndPosShiftNulling_psc(subj, stimType, vGabs, fname)
% WindyMoIndPosShiftNulling.m 
% PSC - 10.10.2020
% TO RUN TESTS: WindyMoIndPosShiftNulling_psc('psc', 'zeb', 1, 'ptest10date.mat')
...
E_min = 3; 
E_max = 15;
E_ncond = 5; % # eccentricity conditions
...
% RF size in visual angle
for i = 1:E_ncond
    if E(i) <= 5.5
        RF_V(i) = sqrt(0.057 * E(i)^(0.15));
    else
        RF_V(i) = sqrt(0.001 * E(i)^(2.48));
    end 
    
    RF_MT(i) = 2.1 * E(i)^(0.47);
end 

% RF d / r in pix
for j = 1:E_ncond 
    d_V(j) = (RF_V(j)/VA_pix);
    r_V(j) = round(0.5 * d_V(j));
    
    d_MT(j) = (RF_MT(j)/VA_pix);
    r_MT(j) = round(0.5 * d_MT(j));
end

%% set Gabor stimulus parameters
...
ept.stimRad = r_R(3); %300;  % radius of the circle of Gabors in pixels, 300 for experiment
gabOffsets = ept.stimRad.*unitcirc(ept.ngabors);

refRectSz = 10;     % reference rectangle inside circle of Gabors (fixation center)

% Texture patch half-size.
% Later on, the 'DrawTextures' command will simply
...
sc = tw/7;%20.0;  % PSC - anyway to get sc properly??
% Frequency of sine grating:
ept.freq = .05;
% ept.contrast of grating:
ept.contrast = 10;  %10
