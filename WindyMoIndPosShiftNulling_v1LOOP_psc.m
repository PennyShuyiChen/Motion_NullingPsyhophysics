function myout = WindyMoIndPosShiftNulling_v1LOOP_psc(subj, stimType, vGabs, fname)
% WindyMoIndPosShiftNulling.m
% PSC - 11.12.2020 - Horizontal test 
% TO RUN TESTS: WindyMoIndPosShiftNulling_v1LOOP_psc('tt6', 'zeb', 1, 'tt6.mat')
...
%% Rig params - PSC
vdist = 15.0; % in cm (closer vdist help to present the stimuli (?)) % 57.3/56.7
...
nrepeat = 20; % 20 repeat / condition 
...
refRectSz = [5,7,8,10];     % reference rectangle inside circle of Gabors (fixation center)
...
pSF = exp(-0.49 * log(E) + log(1.99*3.1)); % cpd
pSF = (pSF) * VA_pix; % dpp for the rig -> cpp (?)
% Spatial constant of the exponential "hull"
BW = 0.6808 + 0.0930*log(E);%3.7268 + 0.6458 * log(E); % bw in octave
SC = bw2sig(pSF, BW);
% Contrast of grating:
ept.contrast = 10;  %10
